...
## PROJECT DIRECTORY - B
df.to_csv('../data1/sample1.csv', index=False, quoting=1)
df.to_csv('../data1/sample2.csv', index=False, quoting=1)
df.to_csv('../data1/sample3.csv', index=False, quoting=1)
df.to_csv('../data1/sample4.csv', index=False, quoting=1)
...
print(lastCSdf)
lastCSdf.to_csv('../data1/out_lastcs.csv', index=False, quoting=1)
...
print(mergedf)
mergedf.to_csv('../data1/out_outermerge.csv', index=False, quoting=1)

# 5. Filter rows based on aggregations

pandas.set_option('expand_frame_repr', False)
print(df)
df = pandas.read_csv('../data1/sample3.csv', dtype=object, parse_dates=['Email'])
...
print(outputdf)
outputdf.to_csv('../data1/out_oldest_person_per_address.csv', index=False, quoting=1)
...
print(df)
df.to_csv('../data1/out_noted_if_is_oldest_per_address.csv', index=False, quoting=1)
# ================================================================================================
...
import numpy
pandas.set_option('expand_frame_repr', False)
df = pandas.read_csv('../data1/sample4.csv')
print(df)
df['Company'] = 'Program' + df['Company']
non_program_columns = list(filter(lambda x: x!= 'Company', df.keys()))
pivotdf = pandas.pivot_table(df, index=non_program_columns, columns='Company', aggfunc=numpy.size)
pivotdf[pandas.notnull(pivotdf)] = 'Registered'
pivotdf.reset_index(inplace=True)
print(pivotdf)
pivotdf.to_csv('../data1/out_pivoted_program_registrations.csv', index=False, quoting=1)
...
print(concatdf)
concatdf.to_csv('../data1/out_concatenated_unique_names.csv', index=False, quoting=1)

print("==============================|Precessory|-Page-132|====================================")
